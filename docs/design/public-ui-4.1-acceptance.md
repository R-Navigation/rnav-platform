...
- 首轮最终线上可视截图曾因本机锁屏暂停，未绕过锁屏；该阻塞已在本次最终验收中解除，七页生产截图均已补齐，见下文。
...
- 发布产物对应提交 `8e4580771e8a80743fd8bd5e6f7506ccd974d389`。服务器通过 SSH 拉取最新代码，在独立 release 目录安装依赖和构建，健康检查后切换；旧 Next/Server 产物保留在同一备份目录，支持回滚。
- **最终线上视觉验收已完成**：`rnav41-live-01-home.png` 至 `rnav41-live-07-contact.png`，七页逐页截图、打开并对照对应 concept 后再检查下一页。首页方向/平台/成员真实图片正常；团队 32 张公开成员卡片、6 个纵向身份组及单张校园首图正常；浏览器 Console 无错误。最终团队补充 1440/1024/390 和 768px 英文截图，后台编辑预览采用相同共享组件。
- 最终公开七页、`/monitor`、`/login`、`/sitemap.xml` 和健康接口均通过；22 个不同公开图片地址按受限队列容量顺序检查均为 200，20 个静态资源均为 200。一次 22 请求的突发检查触发既有 4 并发/16 在途上限，超额返回 503，随后顺序复验全部成功；未放宽保护。临时验收地址 `/qa-member` 在线返回 404。
- 服务 `active`、自动重启计数 0，健康接口 `database: ok`。部署后数据计数仍为 users 38 / lab_assets 76 / procurement_requests 32 / schema_migrations 36；本次未执行任何数据库迁移、seed、import、reset 或成员/页面生产写入。
- 截图索引 `outputs/public-ui-4.1/index.html` 已更新为线上最终截图；保留首轮截图便于历史对照。桌面其他六页只更新首图，不重复变更已验收的内容结构；真实空态及与概念图示意数据的差异仍按前文说明。
