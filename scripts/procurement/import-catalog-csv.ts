...
export async function applyCatalogImport(client: Queryable, items: CatalogImportItem[]) {
  ...
  try {
    ...
    await client.query(`INSERT INTO audit_logs (actor_id, action, target_type, target_id, detail)
      VALUES (NULL, 'procurement.catalog.bulk_import', 'procurement_catalog', 'jd-csv', $1::jsonb)`, [JSON.stringify({ rows: items.length, inserted, updated, source: "jd" })]);
...
export async function importCatalogCsv({ input, report, databaseUrl, apply }: { input: string; report: string; databaseUrl?: string; apply: boolean }) {
  ...
  if (apply) {
    ...
    try { result = await applyCatalogImport(client, items); }
    finally { await client.end(); }
  }
  await writeJson(report, { ...summary, mode: apply ? "applied" : "preview", result });
  return { ...summary, mode: apply ? "applied" : "preview", result, report: resolve(report) };
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const args = parseArgs();
  importCatalogCsv({ input: requiredArg(args, "in"), report: typeof args.get("report") === "string" ? String(args.get("report")) : "outputs/procurement-catalog-import-report.json", databaseUrl: typeof args.get("database-url") === "string" ? String(args.get("database-url")) : process.env.DATABASE_URL, apply: args.has("apply") })
