...
import { normalizeMonitorData, transformUsers } from "./migrate-legacy.js";
...
test("monitor devices normalize legacy nullable descriptions", () => {
  const result = normalizeMonitorData({ devices: [{ id: "d1", description_zh: null, description_en: null }] });
  assert.deepEqual(result.devices, [{ id: "d1", description_zh: "", description_en: "" }]);
});
