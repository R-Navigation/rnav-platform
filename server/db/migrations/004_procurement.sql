CREATE TABLE IF NOT EXISTS procurement_requests (
  ...
  total_estimated_amount numeric(12,2) NOT NULL DEFAULT 0 CHECK (total_estimated_amount >= 0),
...
CREATE TABLE IF NOT EXISTS procurement_status_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  request_id uuid NOT NULL REFERENCES procurement_requests(id) ON DELETE CASCADE,
  from_status text CHECK (from_status IS NULL OR from_status IN ('draft', 'submitted', 'approved', 'rejected', 'purchasing', 'purchased', 'received', 'closed', 'cancelled')),
  to_status text NOT NULL CHECK (to_status IN ('draft', 'submitted', 'approved', 'rejected', 'purchasing', 'purchased', 'received', 'closed', 'cancelled')),
