...
export function createMediaRouter({ authMiddleware, service, trustProxy, maxBytes, publicBaseUrl, pathPrefix }: { authMiddleware: RequestHandler; service: MediaService; trustProxy: boolean; maxBytes: number; publicBaseUrl: string; pathPrefix: string }) {
  ...
  const requireUploadPermission: RequestHandler = (req,res,next) => req.authUser?.permissions.some((permission) => permission === "site.media.write" || permission === "profile.write_own" || permission === "site.members.write") ? next() : res.status(403).json({error:"Permission denied"});
