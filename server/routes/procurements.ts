...
export function createProcurementRouter({ authMiddleware, service, trustProxy }: { authMiddleware: RequestHandler; service: ProcurementService; trustProxy: boolean }) {
  ...
  router.delete("/api/procurements/catalog/categories/:id", sameOrigin, async (request, response, next) => { try { if (!request.authUser!.permissions.includes("procurements.purchase")) throw new ProcurementAccessError("Permission denied"); response.json(await service.deleteCatalogCategory(procurementIdSchema.parse(request.params.id), actor(request))); } catch (error) { handle(response, next, error); } });
  ...
  router.delete("/api/procurements/catalog/items/:id", sameOrigin, async (request, response, next) => { try { if (!request.authUser!.permissions.includes("procurements.purchase")) throw new ProcurementAccessError("Permission denied"); response.json(await service.deleteCatalogItem(procurementIdSchema.parse(request.params.id), actor(request))); } catch (error) { handle(response, next, error); } });
