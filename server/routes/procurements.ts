...
export function createProcurementRouter({ authMiddleware, service, trustProxy }: { authMiddleware: RequestHandler; service: ProcurementService; trustProxy: boolean }) {
  ...
  router.get("/api/procurements/catalog", async (request, response, next) => { try { response.json(await service.listCatalog(catalogQuerySchema.parse(request.query))); } catch (error) { handle(response, next, error); } });
