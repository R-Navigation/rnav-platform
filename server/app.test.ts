...
test("unified app exposes health and mounts feature routers before Next fallback", async () => {
  ...
  const app = createApp({ health: { database: async () => true }, routers: [], nextHandler: (request, response) => { nextRequests.push(request.url); response.status(418).end(); } });
  ...
  try {
    ...
    assert.equal((await fetch(`${base}/some-page`)).status, 418); assert.deepEqual(nextRequests, ["/some-page"]);
