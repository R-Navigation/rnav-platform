  ...
  return {
    async listUsers(query: { search?: string; status?: string; tier?: string }) {
      const result = await pool.query<any>(`SELECT users.id,users.username,users.email,users.display_name,users.base_tier,users.status,
        ...
        WHERE ($1='' OR users.username ILIKE '%'||$1||'%' OR users.display_name ILIKE '%'||$1||'%' OR COALESCE(users.email,'') ILIKE '%'||$1||'%')
          AND ($2='' OR users.status=$2) GROUP BY users.id,user_profiles.user_id ORDER BY users.status,users.username`, [query.search ?? "", query.status ?? ""]);
      return result.rows.map((row: any) => { const permissions = resolvePermissions({ baseTier: row.base_tier, explicitGrants: [...row.template_permissions, ...row.grants], explicitRevokes: row.revokes }).permissions; return {
        ...
        lastLoginAt: row.last_login_at, createdAt: row.created_at, publicVisible: row.public_visible,
        memberStatus: row.member_status, memberCategory: row.member_category, degreeLevel: row.degree_level
    ...
    async createUser(body: z.output<typeof createUserSchema>, actor: Actor) {
      ...
      try { await client.query("BEGIN"); await client.query("SELECT pg_advisory_xact_lock($1)", [SUPER_LOCK]);
        ...
        const memberStatus = body.memberCategory === "alumni" ? "alumni" : "current";
        const degreeLevel = body.memberCategory === "advisor" ? "faculty" : body.memberCategory === "alumni" ? "master" : body.memberCategory;
        await client.query(`INSERT INTO user_profiles(user_id,member_slug,member_category,member_status,degree_level,name_zh,name_en,email,public_visible)
          VALUES($1,$2,$3,$4,$5,$6,$7,$8,false)`, [id, slug(body.username), body.memberCategory, memberStatus, degreeLevel, body.nameZh, body.nameEn, body.email]);
    ...
    async setPublicProfile(userId:string,input:{publicVisible:boolean},actor:Actor){const client=await pool.connect();try{await client.query("BEGIN");const result=await client.query("UPDATE user_profiles SET public_visible=$2,updated_at=now() WHERE user_id=$1",[userId,input.publicVisible]);if(!result.rowCount)throw new UserAdminError("User not found","NOT_FOUND",404);await client.query("INSERT INTO audit_logs(actor_id,action,target_type,target_id,detail) VALUES($1,'user.public_profile','user',$2,$3::jsonb)",[actor.id,userId,JSON.stringify(input)]);await client.query("COMMIT");}catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}},
    ...
    async revokeSessions(userId:string,actor:Actor){const target=await pool.query<{base_tier:BaseTier}>("SELECT base_tier FROM users WHERE id=$1",[userId]);if(!target.rows[0])throw new UserAdminError("User not found","NOT_FOUND",404);if(target.rows[0].base_tier==='super'&&actor.baseTier!=='super')throw new UserAdminError("Only super can revoke super sessions","SUPER_REQUIRED",403);const result=await pool.query("DELETE FROM session_tokens WHERE user_id=$1",[userId]);await pool.query("INSERT INTO audit_logs(actor_id,action,target_type,target_id,detail) VALUES($1,'user.sessions.revoke','user',$2,$3::jsonb)",[actor.id,userId,JSON.stringify({count:result.rowCount??0})]);return{revoked:result.rowCount??0};}
  };
}
