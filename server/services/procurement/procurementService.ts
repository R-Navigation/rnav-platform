...
export function createProcurementService(pool: Pick<Pool, "connect" | "query">, dependencies: Dependencies = {}) {
  ...
  return {
    ...
    async getRequest(id: string, actor: Actor) {
      ...
      const [items,comments,history,spendingEntries]=await Promise.all([pool.query(`SELECT request_items.*,
        ...
        WHERE request_items.request_id=$1 ORDER BY request_items.sort_order,request_items.id`,[id]),pool.query("SELECT comments.*,users.display_name author_name FROM procurement_comments comments JOIN users ON users.id=comments.author_id WHERE request_id=$1 ORDER BY comments.created_at",[id]),pool.query("SELECT history.*,users.display_name actor_name FROM procurement_status_history history JOIN users ON users.id=history.actor_id WHERE request_id=$1 ORDER BY history.created_at",[id]),pool.query(`SELECT entries.id,entries.scope,entries.amount,entries.note,
          ...
          WHERE entries.request_id=$1 GROUP BY entries.id ORDER BY entries.created_at,entries.id`,[id])]);
      return {...request,items:items.rows,comments:comments.rows,history:history.rows,spending_entries:spendingEntries.rows.map((entry:any)=>({...entry,amount:Number(entry.amount)}))};
    },
