...
function client(rows: Record<string, unknown>[] = []) {
  ...
  const value = {
    ...
    query: async (sql: string, values?: readonly unknown[]) => {
      ...
      if (/RETURNING id,\s*request_no/.test(sql))
        return {
          rows: [
            {
              id: "00000000-0000-4000-8000-000000000099",
              request_no: "PR-20260712-000001",
            },
          ],
          rowCount: 1,
        };
      if (
        sql.includes("FROM procurement_catalog_items") &&
...
test("creating a submitted request persists items, history, and audit atomically", async () => {
  ...
  const result = await service.createRequest(
    {
      ...
      reason: "实验",
      items: [
        { itemName: "D455", spec: "", quantity: 2, estimatedUnitPrice: 1500 },
      ],
    },
  ...
  assert.ok(
    db.queries.some((query) => query.sql.includes("procurement_request_items")),
  );
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("procurement_status_history"),
    ),
  );
...
test("catalog items are resolved by the server and persisted with an immutable snapshot", async () => {
  ...
  const insert = db.queries.find((query) =>
    query.sql.includes("INSERT INTO procurement_request_items"),
  )!;
  ...
  const db = client([
    {
      ...
      unit: "件",
      pack_size: 1,
      estimated_unit_price: "9999999.99",
      vendor: null,
      url: null,
      category_code: "other-standard",
      category_name_zh: "其他标准件",
    },
  ]);
  const service = createProcurementService({
    connect: async () => db,
...
test("inactive catalog visibility requires purchase permission", async () => {
  ...
  await assert.rejects(
    service.listCatalog(
      ...
      { id: "actor", permissions: ["procurements.create"] },
    ),
    ProcurementAccessError,
  );
});
...
test("catalog filters bind subcategory and attribute selections as JSONB predicates", async () => {
  ...
  await service.listCatalog(
    {
      ...
      attributes: { thread: ["M3", "M4"], variants: ["黑色"] },
      includeInactive: false,
      limit: 10,
      offset: 0,
    },
  ...
  assert.ok(
    itemQuery.values?.some(
      (value) => Array.isArray(value) && value.includes("M3"),
    ),
  );
...
test("catalog maintenance requires procurement purchase permission", async () => {
  ...
  await assert.rejects(
    service.deleteCatalogItem("00000000-0000-4000-8000-000000000011", {
      id: "actor",
      permissions: [],
    }),
    ProcurementAccessError,
...
test("catalog writes are auditable and duplicate codes become stable conflicts", async () => {
    ...
    query: async (sql: string) => {
      ...
      return {
        rows: sql.includes("RETURNING *")
          ? [{ id: "category-1", code: "bolts", name_zh: "螺栓" }]
          : [],
        rowCount: 1,
      };
    },
  ...
  await service.createCatalogCategory(
    {
      code: "bolts",
      nameZh: "螺栓",
      nameEn: "",
  ...
  assert.ok(
    calls.some((sql) => sql.includes("procurement_catalog_subcategories")),
  );
  ...
  const duplicate = createProcurementService({
    connect: async () => duplicateDb,
  } as never);
  await assert.rejects(
    duplicate.createCatalogCategory(
      {
        code: "bolts",
        nameZh: "螺栓",
        nameEn: "",
...
test("catalog item deletion is auditable and preserves request history through the database foreign key", async () => {
  ...
  const result = await service.deleteCatalogItem(
...
test("request details backfill missing historical category fields without overriding snapshots", async () => {
  ...
  const service = createProcurementService({
    query: async (sql: string) => {
      calls.push(sql);
      if (sql.includes("FROM procurement_requests requests"))
        return {
          rows: [
            { id: "request-1", requester_id: "actor", status: "submitted" },
          ],
          rowCount: 1,
        };
      if (sql.includes("FROM procurement_request_items request_items"))
        return {
          rows: [
            {
              id: "item-1",
  ...
  const detail = await service.getRequest("request-1", {
    id: "actor",
    permissions: ["procurements.read_own"],
  });
  const itemQuery = calls.find((sql) =>
    sql.includes("FROM procurement_request_items request_items"),
  )!;
  assert.match(itemQuery, /LEFT JOIN procurement_catalog_subcategories/);
  assert.match(itemQuery, /jsonb_build_object/);
  assert.match(
    itemQuery,
    /\|\| jsonb_strip_nulls\(COALESCE\(request_items\.catalog_snapshot/,
  );
  assert.equal(
    detail.items[0].catalog_snapshot.subcategoryNameZh,
    "内六角圆柱头螺钉",
  );
});
...
test("catalog category deletion rejects non-empty categories", async () => {
  ...
  const service = createProcurementService({
    query: async (sql: string) => {
      calls.push(sql);
      if (sql.includes("DELETE FROM procurement_catalog_categories"))
        return { rows: [], rowCount: 0 };
      return { rows: [{ exists: true }], rowCount: 1 };
    },
  } as never);
  await assert.rejects(
    service.deleteCatalogCategory("00000000-0000-4000-8000-000000000012", {
      id: "actor",
      permissions: ["procurements.purchase"],
    }),
    (error: unknown) =>
      error instanceof ProcurementConflictError &&
      error.message.includes("先删除"),
  );
...
test("a requester cannot cancel another member's submitted request", async () => {
  const db = client([
    { id: "request-1", requester_id: "other-user", status: "submitted" },
  ]);
  const service = createProcurementService({
    connect: async () => db,
  } as never);
  await assert.rejects(
    service.transition(
      ...
      { action: "cancel", note: "" },
      { id: "actor", permissions: ["procurements.read_own"] },
    ),
    ProcurementAccessError,
  );
  assert.equal(db.queries.at(-1)?.sql, "ROLLBACK");
...
test("invalid current state rolls back without status history", async () => {
  const db = client([
    { id: "request-1", requester_id: "other", status: "closed" },
  ]);
  const service = createProcurementService({
    connect: async () => db,
  } as never);
  await assert.rejects(
    service.transition(
      "00000000-0000-4000-8000-000000000099",
      { action: "approve", note: "" },
      { id: "actor", permissions: ["procurements.review"] },
    ),
    ProcurementConflictError,
  );
  assert.equal(
...
test("transition updates bind exactly the placeholders used by each status", async () => {
  for (const [status, action] of [
    ...
    await service.transition(
      ...
      {
        id: "actor",
        permissions: ["procurements.purchase", "procurements.close"],
      },
    );
    const update = db.queries.find((query) =>
      query.sql.startsWith("UPDATE procurement_requests"),
    )!;
    const placeholders = [...update.sql.matchAll(/\$(\d+)/g)].map((match) =>
      Number(match[1]),
    );
...
function processingClient(
  ...
  return {
    ...
    async query(sql: string, values?: readonly unknown[]) {
      queries.push({ sql, values });
      if (
        sql.includes("FROM procurement_requests") &&
        sql.includes("FOR UPDATE")
      )
        return {
          rows: [{ id: "request-1", status: requestStatus }],
          rowCount: 1,
        };
      if (sql.includes("SELECT processing_status"))
        return {
          rows: itemStatuses.map((processing_status) => ({
            processing_status,
          })),
          rowCount: itemStatuses.length,
        };
      if (sql.includes("INSERT INTO procurement_spend_entries"))
        return { rows: [{ id: "spend-entry-1" }], rowCount: 1 };
      return { rows: [], rowCount: 1 };
    },
    release() {},
  };
...
test("saving line processing persists progress and moves a submitted request into processing", async () => {
  ...
  const result = await service.saveProcessing(
    "00000000-0000-4000-8000-000000000099",
    {
      items: [
        {
  ...
  assert.ok(
    db.queries.some((query) => query.sql.includes("processing_status=$3")),
  );
  assert.ok(db.queries.some((query) => query.sql.includes("$6::timestamptz")));
  assert.ok(
    db.queries.some((query) => query.sql.includes("status='purchasing'")),
  );
});
...
test("saving line processing also starts an approved request", async () => {
  ...
  assert.equal(result.status, "purchasing");
  const history = db.queries.find(
    (query) =>
      query.sql.includes("INSERT INTO procurement_status_history") &&
      query.sql.includes("'purchasing'"),
  );
...
test("saving processing replaces grouped spending records atomically", async () => {
  ...
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("DELETE FROM procurement_spend_entries"),
    ),
  );
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("INSERT INTO procurement_spend_entries"),
    ),
  );
  assert.equal(
    db.queries.filter((query) =>
      query.sql.includes("INSERT INTO procurement_spend_entry_items"),
    ).length,
    2,
  );
});
    ...
    service.saveProcessing(
      "00000000-0000-4000-8000-000000000099",
      {
        ...
        spendingEntries: [
          {
            scope: "items",
            itemIds: ["00000000-0000-4000-8000-000000000011"],
            amount: 10,
            note: "",
          },
        ],
...
test("processing cannot complete while lines are pending", async () => {
  ...
  await assert.rejects(
    service.completeProcessing("00000000-0000-4000-8000-000000000099", {
      id: "actor",
      permissions: ["procurements.purchase"],
    }),
    ProcurementConflictError,
...
test("processing completion orders requests with purchases and closes all-rejected requests", async () => {
  for (const [lineStatuses, expected] of [
    [["purchased", "rejected"], "purchased"],
...
test("processing completion atomically persists the submitted draft", async () => {
  ...
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("procurement.processing.complete"),
    ),
  );
  assert.equal(
    db.queries.some((query) =>
      query.sql.includes("procurement.processing.save"),
    ),
    false,
  );
});

test("confirming receipt advances an ordered request to received", async () => {
  ...
  assert.ok(
    db.queries.some((query) => query.sql.includes("'purchased','received'")),
  );
});

test("requester revisions replace lines and resubmit the same request atomically", async () => {
  const db = client([
    {
      id: "request-1",
      request_no: "PR-1",
  ...
  const result = await service.reviseRequest(
    "00000000-0000-4000-8000-000000000099",
    {
      title: "修订后的申请",
      reason: "已补充型号",
      items: [{ itemName: "D455", spec: "USB-C", quantity: 1 }],
    },
    { id: "actor", permissions: ["procurements.create"] },
  );
  ...
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("DELETE FROM procurement_request_items"),
    ),
  );
  assert.ok(
    db.queries.some((query) =>
      query.sql.includes("'revision_requested','submitted'"),
    ),
  );
  assert.ok(
    db.queries.some((query) => query.sql.includes("procurement.revise")),
  );
  assert.equal(db.queries.at(-1)?.sql, "COMMIT");
});
