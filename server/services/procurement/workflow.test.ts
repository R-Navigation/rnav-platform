...
test("procurement workflow enforces the review, purchase, and close stages", () => {
  ...
  assert.equal(
    validateTransition("submitted", "request_revision"),
    "revision_requested",
  );
...
test("each privileged transition maps to one granular permission", () => {
  assert.equal(
    requiredPermissionForTransition("approve"),
    "procurements.review",
  );
  assert.equal(
    requiredPermissionForTransition("request_revision"),
    "procurements.review",
  );
  assert.equal(
    requiredPermissionForTransition("start_purchase"),
