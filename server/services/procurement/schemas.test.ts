...
import { catalogItemSchema, catalogQuerySchema, createProcurementSchema, processingSaveSchema, transitionSchema } from "./schemas.js";
...
test("processing amounts support single, grouped, and request-total records", () => {
  ...
  const items = [{ itemId: first, status: "purchased" }, { itemId: second, status: "purchased" }];
  assert.equal(processingSaveSchema.safeParse({ items, spendingEntries: [{ scope: "items", itemIds: [first], amount: 12.5 }, { scope: "items", itemIds: [second], amount: 8 }] }).success, true);
  assert.equal(processingSaveSchema.safeParse({ items, spendingEntries: [{ scope: "items", itemIds: [first, second], amount: 20.5, note: "合并付款" }] }).success, true);
  assert.equal(processingSaveSchema.safeParse({ items, spendingEntries: [{ scope: "request_total", amount: 20.5 }] }).success, true);
});
