...
export function createPostgresPublicSiteRepository(pool: Pick<Pool, "query">): PublicSiteRepository {
  return {
    ...
    async getFacilityItems() {
      ...
      return items.rows.map((row) => ({ category: row.category_key, sortOrder: row.sort_order, icon: row.icon, tag: locale(row, "tag"), title: locale(row, "title"), description: locale(row, "description"), specLine: locale(row, "spec_line"), image: image(row), specs: (specMap.get(row.id) ?? []).map((item: PublicRecord) => ({ label: locale(item, "label"), value: locale(item, "value") })) }));
    },
