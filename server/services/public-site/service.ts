...
const text = (
  ...
  return {
    zh: String(fallback.zh || "").trim(),
    en: single || String(fallback.en || "").trim(),
  };
};
const object = (value: unknown): PublicRecord =>
  value && typeof value === "object" && !Array.isArray(value)
    ...
    : {};
const array = (value: unknown): PublicRecord[] =>
...
const itemFields: Allowlist = {
  ...
  href: true,
  image: imageFields,
  pdf: { assetId: true, src: true, label: localized },
  links: [linkFields],
  contacts: [{ label: localized, value: localized }],
  keywords: [localized],
...
const facilitySectionFields: Allowlist = {
  ...itemFields,
  subtitle: localized,
  items: [itemFields],
...
const pageAllowlists: Record<string, Allowlist> = {
  ...
  home: {
    ...
    sections: {
      researchAreasTitle: localized,
      researchAreasCta: localized,
      featuredEyebrow: localized,
      featuredTitle: localized,
      archiveLabel: localized,
      newsEyebrow: localized,
      newsTitle: localized,
    },
    researchAreas: [itemFields],
  ...
  research_page: {
    ...
    ui: itemFields,
    filters: [itemFields],
    topicLabels: [itemFields],
    typeLabels: [itemFields],
    topicOrder: [true],
    typeOrder: [true],
    footerGraphicText: localized,
  },
  news_page: { header: headerFields },
  team_page: {
    header: headerFields,
    sectionTitles: itemFields,
    recruitment: {
      title: localized,
      description: localized,
      buttonLabel: localized,
  ...
  facilities_page: {
    ...
    categoryOrder: [true],
    sectionTitles: {
      quadrupeds: localized,
      groundVehicles: localized,
      aerialPlatforms: localized,
      handheldSensors: localized,
    },
    sectionConfig: {
      quadrupeds: facilitySectionFields,
      groundVehicles: facilitySectionFields,
      aerialPlatforms: facilitySectionFields,
...
function project(value: unknown, allowlist: Allowlist): unknown {
  ...
  if (Array.isArray(allowlist))
    return Array.isArray(value)
      ? value.map((item) => project(item, allowlist[0]))
      : [];
  const source = object(value),
    result: PublicRecord = {};
  for (const [key, nested] of Object.entries(allowlist))
    if (key in source) result[key] = project(source[key], nested);
  return result;
}
...
export const defaults = {
  site: {
    ...
    footerDescription: {
      zh: "推进导航与具身智能研究。",
      en: "Advancing the frontiers of navigation and AI.",
    },
    footerCopyright: {
      zh: "© 2024 R-Nav 研究组 | 武汉大学 LIESMARS",
      en: "© 2024 R-Nav Research Group | LIESMARS, Wuhan University",
    },
    footerLinks: [],
  },
  home: {
    hero: {
      ...
      description: {
        zh: "我们面向自主系统感知、建图与交互中的真实挑战，研发鲁棒、可扩展的算法与系统。",
        en: "We develop robust, scalable algorithms for autonomous systems to perceive, map, and interact with complex environments.",
      },
      actions: [],
      image: null,
      status: { zh: "", en: "" },
    ...
    sections: {
      ...
      researchAreasCta: { zh: "查看项目", en: "View Projects" },
      featuredEyebrow: { zh: "近期成果", en: "RECENT WORK" },
      featuredTitle: { zh: "代表论文", en: "Featured Publication" },
      archiveLabel: { zh: "全部论文", en: "Archive" },
      newsEyebrow: { zh: "最新动态", en: "UPDATE STREAM" },
      newsTitle: { zh: "近期新闻", en: "Latest News" },
    },
    researchAreas: [],
  ...
  research: {
    header: {
      ...
      description: {
        zh: "展示团队在 SLAM、机器人感知与多智能体协同方面的代表性科研成果。",
        en: "Our scientific contributions to SLAM, robot perception, and multi-agent coordination.",
      },
    },
    ui: { emptyTitle: { zh: "暂无论文", en: "No publications" } },
    filters: [],
    topicLabels: [],
    typeLabels: [],
    topicOrder: [],
  ...
  news: {
    header: {
      ...
      description: {
        zh: "跟踪团队在具身智能与韧性导航方向的研究突破、学术进展与合作成果。",
        en: "Stay informed about our latest research breakthroughs, academic milestones, and collaborations.",
      },
    },
  },
  team: {
    header: {
      ...
      description: {
        ...
        en: "Combining expertise in robotics, computer vision, and embodied intelligence.",
      },
    },
    sectionTitles: {
      ...
      alumni: { zh: "毕业生", en: "Graduates" },
    },
    recruitment: {
      title: { zh: "欢迎联系我们", en: "Join Our Team" },
      description: {
        zh: "我们持续欢迎对机器人与导航研究有兴趣的同学与合作伙伴联系。",
        en: "We welcome students and collaborators passionate about robotics and navigation.",
      },
  ...
  facilities: {
    header: {
      ...
      description: {
        zh: "团队维护多类机器人平台与高精度传感设备，用于复杂环境下的韧性导航研究。",
        en: "The group maintains robotic platforms and high-precision sensors for resilient navigation research.",
      },
    },
    facilitySections: [],
    cta: {
      ...
      description: {
        zh: "如需使用团队硬件开展合作研究或数据采集，可联系我们。",
        en: "Contact us about collaborative research or dataset collection.",
      },
      buttonLabel: { zh: "联系我们", en: "Contact Us" },
      buttonHref: "/contact",
    },
  ...
  contact: {
    header: {
      ...
      title: {
        zh: "联系 R-Nav 研究组",
        en: "Contact the R-Nav Research Group",
      },
      description: {
        zh: "可通过本页获取团队合作、招生、数据集与设备等联系信息。",
        en: "Reach our lab for collaboration, student opportunities, datasets, and facilities access.",
      },
    },
    sectionTitles: {
    ...
    introText: {
      zh: "本页展示团队公开联系信息。",
      en: "The team can be reached through the channels below.",
    },
    heroImage: null,
    primaryChannels: [],
    socialLinks: [],
...
function mergeLocalized(base: PublicRecord, stored: unknown): PublicRecord {
  ...
  for (const [key, fallback] of Object.entries(base)) {
    ...
    if (
      ...
      ("zh" in fallback || "en" in fallback)
    )
      result[key] = text(value, fallback as LocaleText);
    else if (
      fallback &&
      typeof fallback === "object" &&
      !Array.isArray(fallback)
    )
      result[key] = mergeLocalized(fallback, value);
...
function normalizeItem(item: PublicRecord): PublicRecord {
  ...
  for (const key of ["authors", "links", "contacts", "keywords", "specs"])
    if (key in item) normalized[key] = array(item[key]).map(normalizeItem);
  if (normalized.href !== undefined)
    normalized.href = sanitizeActionUrl(normalized.href);
  if (normalized.image) normalized.image = normalizeImage(normalized.image);
  if (normalized.pdf) {
    const pdf = object(normalized.pdf),
      src = sanitizePublicUrl(pdf.src);
    normalized.pdf = src
      ? { assetId: clean(pdf.assetId), src, label: text(pdf.label) }
      : null;
  }
  return normalized;
}
...
function normalizeImage(value: unknown) {
  const source = object(value),
    src = sanitizePublicUrl(source.src);
  return src
    ? {
        assetId: clean(source.assetId),
        src,
        alt: clean(source.alt),
        dataAlt: clean(source.dataAlt),
        positionX: Number(source.positionX ?? 50),
        positionY: Number(source.positionY ?? 50),
...
function normalizeFacilitySection(item: PublicRecord): PublicRecord {
  ...
  if ("video" in item) {
    ...
    normalized.video = {
      ...
      url: sanitizePublicUrl(video.url),
      embedUrl: sanitizeEmbedUrl(video.embedUrl),
      poster: normalizeImage(video.poster),
    };
  }
  return normalized;
}
...
function normalizeMember(item: PublicRecord) {
  ...
  if (group === "postdoc" && !("role" in item)) {
    member.role = { zh: "博士后", en: "Postdoctoral Researcher" };
  }
  if (["phd", "master", "undergrad"].includes(group) && !("degree" in item)) {
    const degrees: Record<string, LocaleText> = {
      phd: { zh: "博士", en: "PhD" },
      master: { zh: "硕士", en: "Master" },
      undergrad: { zh: "本科", en: "Undergraduate" },
    };
...
const facilityCategories = [
  ...
  [
    "handheldSensors",
    { zh: "手持与穿戴式传感器", en: "Handheld & Wearable Sensors" },
  ],
] as const;

function buildFacilitySections(config: PublicRecord, items: PublicRecord[]) {
  const defaultOrder = facilityCategories.map(([key]) => key);
  const configuredOrder = Array.isArray(config.categoryOrder)
    ? config.categoryOrder.map((item: unknown) => clean(item)).filter(Boolean)
    : [];
  const order = [
    ...configuredOrder,
    ...defaultOrder.filter((key) => !configuredOrder.includes(key)),
  ];
    ...
    return [
      {
        category,
        subtitle: text(
          metadata.subtitle ?? titleMap[category],
          fallbackTitle.get(category),
        ),
        ...
        video: {
          title: text(metadata.video?.title),
          description: text(metadata.video?.description),
          url: sanitizePublicUrl(metadata.video?.url),
          embedUrl: sanitizeEmbedUrl(metadata.video?.embedUrl),
          poster: normalizeImage(metadata.video?.poster),
        },
        items: categoryItems,
        ...(categoryItems[0] ?? {}),
      },
...
export function createPublicSiteService(repository: PublicSiteRepository) {
  const page = async (
    key: string,
    fallback: PublicRecord,
  ): Promise<PublicRecord> => {
    const stored = project(
      await repository.getPageContent(key),
      pageAllowlists[key],
    );
  ...
  return {
    async getBootstrap() {
      ...
      return {
        ...
        footer: {
          ...
          links: array(site.footerLinks).map(normalizeItem),
        },
      };
    },
    ...
    async getHomepage(): Promise<PublicRecord> {
      ...
      const select = (
        ...
        const normalizedIds = Array.isArray(ids)
          ? ids.map((id) => clean(id)).filter(Boolean)
          : [];
        const normalized = items.map((item) =>
          key === "slug" ? normalizeMember(item) : normalizeItem(item),
        );
        return normalizedIds.length
          ? normalizedIds
              .map((id) => normalized.find((item) => clean(item[key]) === id))
              .filter(Boolean)
          : normalized.slice(0, fallbackLimit);
      };
      const researchIds = Array.isArray(home.featuredResearchIds)
        ? home.featuredResearchIds
        : [];
      const legacyResearchIds = researchIds.length
        ? researchIds
      ...
      return {
        ...
        facilities: {
          facilitySections: [
            {
              items: select(facilities, home.featuredFacilityIds, "id", 3),
            },
          ],
        },
        team: {
          advisors: select(members, home.featuredMemberSlugs, "slug", 8),
        },
        news: { items: select(news, home.newsPreviewIds, "id", 4) },
        contact: { header: contact.header },
      };
    },
    async getResearch(): Promise<PublicRecord> {
      const [config, items] = await Promise.all([
        page("research_page", defaults.research),
        repository.getResearchItems(),
      ]);
      return { ...config, publications: items.map(normalizeItem) };
    },
    async getNews(): Promise<PublicRecord> {
      const [config, items] = await Promise.all([
        page("news_page", defaults.news),
        repository.getNewsItems(),
      ]);
      return { ...config, items: items.map(normalizeItem) };
    },
    async getTeam(): Promise<PublicRecord> {
      const [config, rawMembers] = await Promise.all([
        page("team_page", defaults.team),
        repository.getTeamMembers(),
      ]);
      ...
      const group = (key: string) =>
        members
          ...
          .sort(
            (a, b) =>
              clean(a.enrollmentYear).localeCompare(clean(b.enrollmentYear)) ||
      ...
      return {
        ...
        facultyLead: advisors[0] ?? null,
        advisors,
        postdocs: group("postdoc"),
        phdStudents: group("phd"),
        masterStudents: group("master"),
    ...
    async getFacilities(): Promise<PublicRecord> {
      ...
      const config = sanitizeConfig(
        mergeLocalized(defaults.facilities, projected),
      ) as PublicRecord;
      const legacyItems = rawItems.map(normalizeItem);
      const hasExplicitSections = Object.prototype.hasOwnProperty.call(
        object(raw),
        "facilitySections",
      );
    ...
    async getContact(): Promise<PublicRecord> {
      const [config, contacts] = await Promise.all([
        page("contact_page", defaults.contact),
        repository.getContactItems(),
      ]);
      return {
        ...config,
        primaryChannels: contacts.primaryChannels.map(normalizeItem),
        socialLinks: contacts.socialLinks.map(normalizeItem),
        extraCards: contacts.extraCards.map(normalizeItem),
      };
    },
  };
}
