...
import { createUpstreamHeaders, validateIncomingOrigin, validateRequestSize } from "./proxy";
...
async function proxy(request: Request, context: { params: Promise<{ path?: string[] }> }) {
  ...
  try {
    const upstream = await fetch(getInternalApiUrl(`/api/procurements${suffix}${incoming.search}`), { method: request.method, headers: createUpstreamHeaders(request), body: ["GET", "HEAD"].includes(request.method) ? undefined : await request.arrayBuffer(), cache: "no-store" });
