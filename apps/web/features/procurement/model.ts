...
export type CatalogItem = {
  ...
  nameEn: string;
  spec: string;
  specMetadata: Record<string, string | number | boolean | null | string[]>;
  unit: string;
  packSize: number;
...
export type ProcurementCatalogSnapshot = {
  categoryNameZh?: string;
  subcategoryNameZh?: string;
  specMetadata?: CatalogItem["specMetadata"];
  url?: string;
};
export type GroupableProcurementItem = {
...
export type ProcessingDraftState = {
  status: "pending" | "purchased" | "rejected";
  rejectionReason: string;
};
export type ProcessingSpendingDraft = {
...
export function availableActions(
  ...
  capability: ProcurementCapabilities,
  isRequester: boolean,
) {
  const actions: Array<{ action: ProcurementAction; label: string }> = [];
  if (status === "submitted" && isRequester)
    actions.push({ action: "cancel", label: "撤回申请" });
  if (status === "submitted" && capability.review && !isRequester)
    actions.push(
      { action: "request_revision", label: "退回修改" },
      { action: "approve", label: "批准" },
      { action: "reject", label: "驳回" },
    );
  if (status === "received" && capability.close)
    actions.push({ action: "close", label: "关闭归档" });
  return actions;
}
...
export function addCatalogItem(
  ...
  const key = `catalog:${item.id}`,
    existing = cart.find((entry) => entry.key === key);
...
export function updateCartQuantity(
  ...
  return quantity <= 0
    ? cart.filter((entry) => entry.key !== key)
    : cart.map((entry) => (entry.key === key ? { ...entry, quantity } : entry));
}
...
export function displayAttributes(
  metadata: CatalogItem["specMetadata"],
  definitions: CatalogAttribute[] = [],
) {
  const definitionMap = new Map(
    definitions.map((item) => [item.attributeKey, item]),
  );
  return Object.entries(metadata)
    ...
    .map(([key, value]) => ({
      key,
      label: definitionMap.get(key)?.labelZh ?? fallbackLabels[key] ?? key,
      value: Array.isArray(value) ? value.join("、") : String(value),
      unit: definitionMap.get(key)?.unit ?? unitByKey[key] ?? "",
      sortOrder: definitionMap.get(key)?.sortOrder ?? 999,
    }))
    .sort(
      (a, b) =>
        a.sortOrder - b.sortOrder || a.label.localeCompare(b.label, "zh-CN"),
    );
}

export function groupProcurementItems<T extends GroupableProcurementItem>(
  items: T[],
) {
  ...
  for (const item of items) {
    ...
    const subcategory =
      item.catalog_snapshot?.subcategoryNameZh ||
      (item.source_type === "custom" ? "手动填写" : "未分类");
...
export function processingCompletionBlockers(
  ...
  const pendingCount = itemIds.filter(
    (id) => (draft[id]?.status ?? "pending") === "pending",
  ).length;
  if (!itemIds.length) blockers.push("采购清单中没有可处理的条目。");
  if (pendingCount)
    blockers.push(`还有 ${pendingCount} 个条目未标记为已购买或已驳回。`);
...
export function processingSaveBlockers(
  ...
  const missingReasons = itemIds.filter(
    (id) =>
      draft[id]?.status === "rejected" && !draft[id]?.rejectionReason.trim(),
  ).length;
  return [
    ...(missingReasons
      ? [`还有 ${missingReasons} 个已驳回条目未填写驳回意见。`]
      : []),
...
export function processingSpendingBlockers(
  ...
  const requestTotals = spending.filter(
    (entry) => entry.scope === "request_total",
  );
  const billedItemIds = spending.flatMap((entry) =>
    entry.scope === "items" ? entry.itemIds : [],
  );
  const duplicateBilling = new Set(billedItemIds).size !== billedItemIds.length;
  const invalidBilling = billedItemIds.some(
    (id) => draft[id]?.status !== "purchased",
  );
  const hasPurchasedItems = Object.values(draft).some(
    (item) => item.status === "purchased",
  );
  if (invalidAmounts)
    blockers.push(`还有 ${invalidAmounts} 条实际金额未填写或格式不正确。`);
  if (requestTotals.length && spending.length !== 1)
    blockers.push("整单总额不能与按条目金额同时记录。");
  if (requestTotals.length && !hasPurchasedItems)
    blockers.push("没有已购买条目时不能记录整单实际金额。");
