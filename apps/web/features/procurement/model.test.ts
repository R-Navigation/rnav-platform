...
test("normal members can create and only see requester actions", () => {
  const capability = procurementCapabilities([
    "procurements.create",
    "procurements.read_own",
  ]);
  assert.equal(capability.create, true);
  assert.equal(capability.readAll, false);
  assert.deepEqual(availableActions("submitted", capability, true), [
    { action: "cancel", label: "撤回申请" },
  ]);
});
...
test("advanced procurement permissions expose only their workflow stage", () => {
  ...
  assert.deepEqual(
    availableActions("submitted", reviewer, false).map((item) => item.action),
    ["request_revision", "approve", "reject"],
  );
  assert.deepEqual(availableActions("submitted", reviewer, true), [
    { action: "cancel", label: "撤回申请" },
  ]);
  const purchaser = procurementCapabilities(["procurements.purchase"]);
  assert.deepEqual(availableActions("submitted", purchaser, false), []);
  const closer = procurementCapabilities(["procurements.close"]);
  assert.deepEqual(
    availableActions("received", closer, false).map((item) => item.action),
    ["close"],
  );
});

const bolt: CatalogItem = {
  id: "bolt-1",
  categoryId: "category-1",
  categoryCode: "bolts",
  categoryNameZh: "螺栓",
  subcategoryId: "subcategory-1",
  subcategoryCode: "socket-head-cap-screw",
...
test("setting a cart quantity to zero removes the line", () => {
  assert.deepEqual(
    updateCartQuantity(addCatalogItem([], bolt), "catalog:bolt-1", 0),
    [],
  );
});
  ...
  const grouped = groupProcurementItems([
    {
      ...
      catalog_snapshot: { categoryNameZh: "垫圈", subcategoryNameZh: "平垫圈" },
    },
    {
      id: "bolt",
      source_type: "catalog" as const,
