...
export function ScholarlyProfileEditor({ userId, canEdit }: { userId: string; canEdit: boolean }) {
  ...
  const [candidates, setCandidates] = useState<Candidate[]>([]), [institution, setInstitution] = useState(""), [busy, setBusy] = useState(false), [message, setMessage] = useState(""), [error, setError] = useState("");
