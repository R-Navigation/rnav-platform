...
export function SiteContentConsole({ permissions }: Props) {
  ...
  const availableModules = useMemo(
    () => getAvailableSiteModules(modules, permissions),
    [modules, permissions],
  );
  const selectedModule =
    availableModules.find((module) => module.key === selectedKey) ??
    availableModules[0];
  const dirty = selectedModule
    ? isModuleDirty(selectedModule, draftValue) ||
      isJsonEditorDirty(selectedModule.value, jsonSource)
    : false;
  ...
  const loadSnapshot = useCallback(
    async (preferredKey?: string) => {
      ...
      setMessage("");
      try {
        const response = await fetch("/api/site-admin/snapshot", {
          cache: "no-store",
        });
        ...
        const next =
          available.find((module) => module.key === preferredKey) ??
          available[0];
  ...
  async function saveCurrentModule() {
    ...
    try {
      ...
      const saved = applySavedModule(
        { ...selectedModule, value: draftValue },
        payload.updatedAt,
      );
      setModules((current) =>
        current.map((module) => (module.key === saved.key ? saved : module)),
      );
  ...
  async function resolveConflict(choice: "reload" | "keep-local") {
    ...
    try {
      ...
      if (choice === "reload") {
        ...
        const compareHint = resolved.serverValueChanged
          ? "服务器内容也有变化；"
          : "";
        setMessage(
          `本地草稿已基于最新版本 ${revision} 重新建立基线；${compareHint}请核对后再次保存。`,
        );
      }
    } catch (error) {
  ...
  if (loadState === "loading") {
    return (
      <section aria-busy="true" aria-live="polite">
        <p className="text-sm font-semibold text-cyan-700">官网内容</p>
  ...
  if (loadState === "error") {
    return (
      <section aria-labelledby="site-load-error">
        <p className="text-sm font-semibold text-cyan-700">官网内容</p>
        <h1
        ...
        <button
          className="mt-6 bg-blue-950 px-4 py-2 text-sm font-bold text-white"
          onClick={() => void loadSnapshot(selectedKey)}
          type="button"
        >
          重试
        </button>
  ...
  return (
    <section aria-labelledby="site-console-heading">
      ...
      <fieldset
        aria-busy={editorInteraction.busy}
        className="min-w-0"
      ...
      >
          ...
          <div className="flex min-w-max gap-1" role="tablist">
            {availableModules.map((module) => (
              <button
                ...
                ref={(node) => {
                  if (node) moduleButtonRefs.current.set(module.key, node);
                  else moduleButtonRefs.current.delete(module.key);
                }}
                role="tab"
                type="button"
              >
        ...
        <p
          aria-live="polite"
          className={`mt-6 border-l-2 px-4 py-3 text-sm ${conflict ? "border-amber-500 bg-amber-50 text-amber-950" : message ? "border-cyan-600 bg-white text-slate-700" : "sr-only"}`}
        ...
        {conflict ? (
          <div className="mt-3 flex flex-wrap gap-3">
            ...
            <button
              className="border border-slate-400 bg-white px-4 py-2 text-sm font-bold text-slate-800 disabled:text-slate-400"
              disabled={conflictRefreshing}
        ...
        <p className="mt-6 text-xs leading-5 text-slate-500">
          需要上传或管理图片、PDF 时，请先使用{" "}
          <Link className="text-cyan-800 underline" href="/console/media">
            媒体资源
          </Link>{" "}
          获取资源地址，再在内容中引用。
        </p>
      ...
      {pendingKey ? (
        <DiscardDialog
          ...
          restoreFocusTo={pendingTriggerRef.current}
        />
      ) : null}
    </section>
  );
