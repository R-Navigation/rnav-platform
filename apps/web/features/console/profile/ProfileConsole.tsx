...
type Profile = {
  version: number; username: string; memberCategory: string; memberStatus: "current" | "alumni"; degreeLevel: string;
  publicVisible: boolean; nameZh: string; nameEn: string; email: string; phone: string; bioZh: string; bioEn: string;
...
export function ProfileConsole({ mustChangePassword }: { mustChangePassword: boolean }) {
  ...
  async function save() {
    ...
    try {
      const body = {
        version: draft.version, memberStatus: draft.memberStatus, degreeLevel: draft.degreeLevel,
        nameZh: draft.nameZh, nameEn: draft.nameEn, email: draft.email, phone: draft.phone,
  ...
  return <section className="max-w-5xl">
    <div><p className="text-sm font-semibold text-cyan-800">账号与公开身份</p><h1 className="mt-1 font-serif text-3xl font-bold text-blue-950">个人资料</h1><p className="mt-2 text-sm text-slate-600">管理员决定账号是否列入官网；列入后，具体展示内容由你自己控制。</p></div>
    ...
    {tab === "profile" ? <div className="mt-7 space-y-8">
      <section className="grid gap-5 border-b border-slate-200 pb-8 sm:grid-cols-[160px_1fr]">
        ...
        <div><h2 className="font-semibold text-blue-950">个人照片</h2><p className="mt-2 text-sm text-slate-600">支持 JPG、PNG、WebP。重新上传并保存后，原照片若未被其他业务引用会进入回收站。</p><div className="mt-4 flex flex-wrap gap-3"><label className="cursor-pointer bg-blue-950 px-4 py-2 text-sm font-semibold text-white">{draft.avatarUrl ? "重新上传" : "上传照片"}<input accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={uploadAvatar} type="file" /></label>{draft.avatarAssetId ? <button className="border border-slate-300 px-4 py-2 text-sm" onClick={() => { update("avatarAssetId", null); update("avatarUrl", null); }} type="button">移除照片</button> : null}</div>{draft.avatarUrl ? <div className="mt-5 grid gap-4 sm:grid-cols-3"><Range label="水平位置" max={100} min={0} onChange={(value) => update("avatarPositionX", value)} value={draft.avatarPositionX} /><Range label="垂直位置" max={100} min={0} onChange={(value) => update("avatarPositionY", value)} value={draft.avatarPositionY} /><Range label="缩放" max={3} min={1} onChange={(value) => update("avatarZoom", value)} step={0.05} value={draft.avatarZoom} /></div> : null}</div>
      </section>
      <div className="grid gap-5 sm:grid-cols-2"><ReadOnly label="用户名" value={draft.username} /><label className="text-sm font-medium">成员阶段<select className={input} value={draft.memberStatus} onChange={(event) => update("memberStatus", event.target.value as Profile["memberStatus"])}><option value="current">在读成员</option><option value="alumni">毕业成员</option></select><span className="mt-1 block text-xs text-slate-500">切换为毕业成员后，内部资产与普通采购权限会自动收缩，账号仍可登录维护资料。</span></label>
      ...
      <button className="bg-blue-950 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50" disabled={busy || JSON.stringify(profile) === JSON.stringify(draft)} onClick={save}>{busy ? "保存中..." : "保存资料"}</button>
    </div> : null}
    {tab === "public" ? <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_1.2fr]"><div className="space-y-3"><p className={`p-3 text-sm ${draft.publicVisible ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}>{draft.publicVisible ? "管理员已将你的账号列入官网成员页。" : "当前账号未被管理员列入官网成员页；你的展示设置会被保存，启用后生效。"}</p>{publicOptions.map(([key, label]) => <label className="flex items-center justify-between border-b border-slate-200 py-3 text-sm" key={key}><span>{label}</span><input checked={visible.has(key)} type="checkbox" onChange={(event) => update("publicFields", event.target.checked ? [...draft.publicFields, key] : draft.publicFields.filter((value) => value !== key))} /></label>)}<button className="mt-4 bg-blue-950 px-5 py-2.5 text-sm font-semibold text-white" disabled={busy} onClick={save}>保存公开设置</button></div><article className="border border-slate-200 bg-white p-6"><p className="text-xs font-bold text-cyan-800">公开成员页预览</p>{visible.has("avatar") && draft.avatarUrl ? <AvatarPreview alt="公开照片预览" className="mt-4 w-28" profile={draft} /> : null}<h2 className="mt-3 font-serif text-2xl text-blue-950">{visible.has("name_zh") ? draft.nameZh : ""} <span className="text-base text-slate-500">{visible.has("name_en") ? draft.nameEn : ""}</span></h2>{visible.has("academic") ? <p className="mt-2 text-sm text-slate-600">{alumni ? `${draft.graduationYear}届` : `${draft.enrollmentYear}级`} · {draft.degreeLevel}</p> : null}{visible.has("major") ? <p className="mt-2 text-sm">{draft.majorZh || draft.majorEn}</p> : null}{visible.has("research") ? <p className="mt-5 text-sm"><b>研究方向：</b>{draft.researchInterestsZh || draft.researchInterestsEn}</p> : null}{visible.has("bio") ? <p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-700">{draft.bioZh || draft.bioEn}</p> : null}{visible.has("links") && draft.personalLinks.length ? <div className="mt-5 flex flex-wrap gap-3">{draft.personalLinks.map((link, index) => <span className="text-sm text-cyan-800 underline" key={index}>{link.labelZh || link.labelEn}</span>)}</div> : null}</article></div> : null}
...
function PasswordForm({ busy, error, onSubmit, requiredChange = false }: { busy: boolean; error: string; onSubmit(event: React.FormEvent<HTMLFormElement>): void; requiredChange?: boolean }) { return <section className="max-w-xl"><p className="text-sm font-semibold text-cyan-800">账号安全</p><h1 className="mt-1 font-serif text-3xl font-bold text-blue-950">{requiredChange ? "首次登录修改密码" : "修改密码"}</h1>{requiredChange ? <p className="mt-3 text-sm text-slate-600">当前账号使用临时密码，修改后即可进入控制台。</p> : null}{error ? <p className="mt-5 border border-red-200 bg-red-50 p-3 text-sm text-red-800">{error}</p> : null}<form className="mt-7 space-y-5" onSubmit={onSubmit}><label className="block text-sm font-medium">当前密码<input autoComplete="current-password" className={input} name="currentPassword" required type="password" /></label><label className="block text-sm font-medium">新密码<input autoComplete="new-password" className={input} minLength={8} name="newPassword" required type="password" /><span className="mt-2 block text-xs text-slate-500">至少 8 个字符。</span></label><label className="block text-sm font-medium">确认新密码<input autoComplete="new-password" className={input} minLength={8} name="confirmPassword" required type="password" /></label><button className="bg-blue-950 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50" disabled={busy} type="submit">{busy ? "修改中..." : requiredChange ? "修改密码并继续" : "修改密码"}</button></form></section>; }
